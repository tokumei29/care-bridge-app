...
export function ImageMemosRecordsListScreen() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      const data = await imageMemosApi.list(recipientId);
      setRecords(data);
      setListError(null);
    } catch (e) {
  ...
  }, [isSignedIn, imageMemosApi, recipientId]);
  ...
  const goNew = useCallback(() => {
    router.push({
      pathname: '/care/[recipientId]/image-memos/new',
  ...
  const goEdit = useCallback(
    (id: string) => {
      router.push({
        pathname: '/care/[recipientId]/image-memos/[id]',
  ...
  const onDeleteRecord = useCallback(
    (id: string) => {
      ...
      Alert.alert('記録を削除', 'この画像メモを削除しますか？取り消せません。', [
        { text: 'キャンセル', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              try {
                await imageMemosApi.destroy(recipientId, id);
                await fetchList();
              } catch (e) {
    ...
    [fetchList, isSignedIn, imageMemosApi, recipientId]
  );
            ...
            <ContentRail layout={layout}>
              ...
              {!loading && !listError && records.length === 0 && isSignedIn ? (
                <Text style={[styles.emptyText, { color: c.textSecondary }]}>
                  まだ記録がありません。被介護者トップの「入力」→「画像メモの入力」、または右上の「＋ 新規」から追加できます。
                </Text>
              ) : null}
              {!loading &&
              !listError &&
