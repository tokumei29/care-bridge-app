...
export function SleepRecordEditScreen() {
  ...
  const [draft, setDraft] = useState<SleepRecordDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const rec: SleepRecordRecord = await sleepRecordsApi.show(recipientId, recordId);
        if (!cancelled) {
          setDraft(sleepRecordToDraft(rec));
        }
      } catch (e) {
        if (!cancelled) {
  ...
  }, [isSignedIn, sleepRecordsApi, recipientId, recordId]);
  ...
  }, [draft, isSaving, isSignedIn, sleepRecordsApi, recordId, recipientId, router]);
  ...
  const onSavePress = useCallback(() => {
    ...
    Alert.alert('内容の確認', buildSleepSummaryText(draft, recipient.name), [
      { text: '戻る', style: 'cancel' },
      { text: '保存する', onPress: () => void submitUpdate() },
    ]);
  }, [draft, isSignedIn, recipient, submitUpdate]);

  const setDraftSafe = useCallback((action: SetStateAction<SleepRecordDraft>) => {
    setDraft((prev) => {
  ...
  const onDeletePress = useCallback(() => {
    ...
    Alert.alert('記録を削除', 'この睡眠の記録を削除しますか？この操作は取り消せません。', [
      { text: 'キャンセル', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            setIsDeleting(true);
            try {
              await sleepRecordsApi.destroy(recipientId, recordId);
              router.back();
            } catch (e) {
  ...
  }, [isDeleting, isSignedIn, sleepRecordsApi, recordId, recipientId, router]);
