...
import {
  ...
  createEmptyExcretionDraftFromJapanNow,
...
export function ExcretionRecordCreateScreen() {
  ...
  const initialDraft = useMemo(() => createEmptyExcretionDraftFromJapanNow(), []);
  const [draft, setDraft] = useState<ExcretionRecordDraft>(initialDraft);
  ...
  const submitToRails = useCallback(async () => {
    ...
    try {
      await excretionRecordsApi.create(recipientId, draftToExcretionWritePayload(draft));
      Alert.alert('保存しました', '排泄記録をサーバーに登録しました。', [
  ...
  }, [draft, isSignedIn, isSubmitting, excretionRecordsApi, recipientId, router]);
  ...
  const onSubmit = useCallback(() => {
    ...
    Alert.alert('入力内容の確認', buildExcretionSummaryText(draft, recipient.name), [
