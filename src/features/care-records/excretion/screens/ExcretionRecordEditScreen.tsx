...
import {
  ...
  draftToExcretionWritePayload,
  excretionRecordToDraft,
  type ExcretionRecordDraft,
} from '@/features/care-records/excretion/excretionDraft';
import { useExcretionStackBackHeader } from '@/features/care-records/excretion/useExcretionStackBackHeader';
...
export function ExcretionRecordEditScreen() {
  ...
  useExcretionStackBackHeader(recipientId, c);
  const themeKey = scheme === 'dark' ? 'dark' : 'light';
  const layout = useResponsiveLayout();
  const insets = useSafeAreaInsets();

  const [draft, setDraft] = useState<ExcretionRecordDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const rec: ExcretionRecordRecord = await excretionRecordsApi.show(recipientId, recordId);
        if (!cancelled) {
          setDraft(excretionRecordToDraft(rec));
        }
      } catch (e) {
        if (!cancelled) {
  ...
  }, [isSignedIn, excretionRecordsApi, recipientId, recordId]);
  ...
  }, [draft, isSaving, isSignedIn, excretionRecordsApi, recordId, recipientId, router]);
  ...
  const onSavePress = useCallback(() => {
    ...
    Alert.alert('内容の確認', buildExcretionSummaryText(draft, recipient.name), [
      { text: '戻る', style: 'cancel' },
      { text: '保存する', onPress: () => void submitUpdate() },
    ]);
  }, [draft, isSignedIn, recipient, submitUpdate]);

  const setDraftSafe = useCallback((action: SetStateAction<ExcretionRecordDraft>) => {
    setDraft((prev) => {
  ...
  const onDeletePress = useCallback(() => {
    ...
    Alert.alert('記録を削除', 'この排泄記録を削除しますか？この操作は取り消せません。', [
      { text: 'キャンセル', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            setIsDeleting(true);
            try {
              await excretionRecordsApi.destroy(recipientId, recordId);
              router.back();
            } catch (e) {
  ...
  }, [isDeleting, isSignedIn, excretionRecordsApi, recordId, recipientId, router]);
