...
import type { OtherNoteRecord } from '@/api/types/otherNote';
...
export function OtherNoteRecordEditScreen() {
  ...
  const [draft, setDraft] = useState<OtherNoteRecordDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const rec: OtherNoteRecord = await otherNotesApi.show(recipientId, recordId);
        if (!cancelled) {
          setDraft(otherNoteRecordToDraft(rec));
        }
      } catch (e) {
        if (!cancelled) {
  ...
  }, [isSignedIn, otherNotesApi, recipientId, recordId]);

  const submitUpdate = useCallback(async () => {
    if (!recipientId || !recordId || !isSignedIn || !draft || isSaving) return;
    setIsSaving(true);
    try {
      await otherNotesApi.update(recipientId, recordId, draftToOtherNoteWritePayload(draft));
      Alert.alert('更新しました', '記録を保存しました。', [
  ...
  }, [draft, isSaving, isSignedIn, otherNotesApi, recordId, recipientId, router]);
  ...
  const onSavePress = useCallback(() => {
    ...
    Alert.alert('内容の確認', buildOtherNoteSummaryText(draft, recipient.name), [
      { text: '戻る', style: 'cancel' },
      { text: '保存する', onPress: () => void submitUpdate() },
    ]);
  }, [draft, isSignedIn, recipient, submitUpdate]);

  const setDraftSafe = useCallback((action: SetStateAction<OtherNoteRecordDraft>) => {
    setDraft((prev) => {
      ...
      { text: 'キャンセル', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            setIsDeleting(true);
            try {
              await otherNotesApi.destroy(recipientId, recordId);
              router.back();
            } catch (e) {
  ...
  }, [isDeleting, isSignedIn, otherNotesApi, recordId, recipientId, router]);
