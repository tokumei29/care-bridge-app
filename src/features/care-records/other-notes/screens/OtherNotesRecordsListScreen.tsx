...
export function OtherNotesRecordsListScreen() {
  ...
  const [records, setRecords] = useState<OtherNoteRecord[]>([]);
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      const data = await otherNotesApi.list(recipientId);
      setRecords(data);
      setListError(null);
    } catch (e) {
  ...
  }, [isSignedIn, otherNotesApi, recipientId]);
  ...
  const goNew = useCallback(() => {
    router.push({
      pathname: '/care/[recipientId]/notes/new',
  ...
  const goEdit = useCallback(
    (id: string) => {
      router.push({
        pathname: '/care/[recipientId]/notes/[id]',
  ...
  const onDeleteRecord = useCallback(
    (id: string) => {
      ...
      Alert.alert('記録を削除', 'この記録を削除しますか？取り消せません。', [
        { text: 'キャンセル', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              try {
                await otherNotesApi.destroy(recipientId, id);
                await fetchList();
              } catch (e) {
    ...
    [fetchList, isSignedIn, otherNotesApi, recipientId]
  );
          ...
          ListHeaderComponent={
            <ContentRail layout={layout}>
              <Text style={[styles.lead, { color: c.textSecondary }]}>
                {`${recipient.name}さんの「認知症の様子、その他気づき」の記録です（新しい順・日本時間）。各行の「編集」で修正、「削除」で消せます。新規は右上の「＋ 新規」か、被介護者トップの「入力」から開けます。\n\nタグは「そのときの様子」です。赤い「問題、気になる点あり」（警告マーク付き）は、施設やケアマネに相談したい記録として目立つようにしています。`}
              </Text>
              {!isSignedIn ? (
                <View
                  style={[
              ...
              {!loading && !listError && records.length === 0 && isSignedIn ? (
                <Text style={[styles.emptyText, { color: c.textSecondary }]}>
                  まだ記録がありません。被介護者トップの「入力」→「認知症の様子、その他気づきの入力」、または右上の「＋ 新規」から追加できます。
                </Text>
              ) : null}
              {!loading &&
              !listError &&
