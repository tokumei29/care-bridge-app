...
function formatRehabCardHeadline(startedAt: string, endedAt: string): string {
  ...
  const s = parseIsoToJapanDateTimeParts(startedAt);
  const e = parseIsoToJapanDateTimeParts(endedAt);
  return `${datePart} ${pad2(s.hour)}:${pad2(s.minute)}〜${pad2(e.hour)}:${pad2(e.minute)}`;
}

export function RehabRecordsListScreen() {
  ...
  const [records, setRecords] = useState<RehabRecordRecord[]>([]);
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      const data = await rehabRecordsApi.list(recipientId);
      setRecords(data);
      setListError(null);
    } catch (e) {
  ...
  }, [isSignedIn, rehabRecordsApi, recipientId]);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r) => isRecordedAtOnJapanDate(r.started_at, filterDateKey));
  }, [records, filterDateKey]);

  const goNew = useCallback(() => {
    router.push({
      pathname: '/care/[recipientId]/rehab/new',
  ...
  const goEdit = useCallback(
    (id: string) => {
      router.push({
        pathname: '/care/[recipientId]/rehab/[id]',
  ...
  const onDeleteRecord = useCallback(
    (id: string) => {
      ...
      Alert.alert('記録を削除', 'このリハビリ活動の記録を削除しますか？取り消せません。', [
        { text: 'キャンセル', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              try {
                await rehabRecordsApi.destroy(recipientId, id);
                await fetchList();
              } catch (e) {
    ...
    [fetchList, isSignedIn, rehabRecordsApi, recipientId]
  );
      ...
      <View style={styles.flex}>
        <FlatList
            ...
            <ContentRail layout={layout}>
              ...
              {!loading && !listError && records.length === 0 && isSignedIn ? (
                <Text style={[styles.emptyText, { color: c.textSecondary }]}>
                  まだ記録がありません。被介護者トップの「入力」→「リハビリ活動の入力」、または右上の「＋ 新規」から追加できます。
                </Text>
              ) : null}
              {!loading &&
              !listError &&
          ...
          renderItem={({ item }) => {
            ...
            const contentLine = item.content?.trim() || '（行った内容の記載なし）';
            return (
              <ContentRail layout={layout}>
                <View
                  style={[
                  ...
                  <View style={styles.cardTop}>
                    <Text style={[styles.cardDate, { color: c.text }]}>
                      {formatRehabCardHeadline(item.started_at, item.ended_at)}
                    </Text>
                  </View>
                  <Text style={[styles.cardSub, { color: c.textSecondary }]} numberOfLines={3}>
                    {contentLine}
                  </Text>
