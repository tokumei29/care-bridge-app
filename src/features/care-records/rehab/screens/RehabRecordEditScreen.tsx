...
export function RehabRecordEditScreen() {
  ...
  const [draft, setDraft] = useState<RehabRecordDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const rec: RehabRecordRecord = await rehabRecordsApi.show(recipientId, recordId);
        if (!cancelled) {
          setDraft(rehabRecordToDraft(rec));
        }
      } catch (e) {
        if (!cancelled) {
  ...
  }, [isSignedIn, rehabRecordsApi, recipientId, recordId]);
  ...
  }, [draft, isSaving, isSignedIn, rehabRecordsApi, recordId, recipientId, router]);
  ...
  const onSavePress = useCallback(() => {
    ...
    Alert.alert('内容の確認', buildRehabSummaryText(draft, recipient.name), [
      { text: '戻る', style: 'cancel' },
      { text: '保存する', onPress: () => void submitUpdate() },
    ]);
  }, [draft, isSignedIn, recipient, submitUpdate]);

  const setDraftSafe = useCallback((action: SetStateAction<RehabRecordDraft>) => {
    setDraft((prev) => {
  ...
  const onDeletePress = useCallback(() => {
    ...
    Alert.alert('記録を削除', 'このリハビリ活動の記録を削除しますか？この操作は取り消せません。', [
      { text: 'キャンセル', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            setIsDeleting(true);
            try {
              await rehabRecordsApi.destroy(recipientId, recordId);
              router.back();
            } catch (e) {
  ...
  }, [isDeleting, isSignedIn, rehabRecordsApi, recordId, recipientId, router]);
