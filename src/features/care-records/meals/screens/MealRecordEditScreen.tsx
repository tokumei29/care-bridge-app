...
} from '@/features/care-records/meals/mealDraft';
import { useMealStackBackHeader } from '@/features/care-records/meals/useMealStackBackHeader';
...
export function MealRecordEditScreen() {
  ...
  useMealStackBackHeader(recipientId, c);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const rec: MealRecordRecord = await mealRecordsApi.show(recipientId, recordId);
        if (!cancelled) {
          setDraft(mealRecordToDraft(rec));
        }
      } catch (e) {
        if (!cancelled) {
  ...
  }, [isSignedIn, mealRecordsApi, recipientId, recordId]);
  ...
  }, [draft, isSaving, isSignedIn, mealRecordsApi, recordId, recipientId, router]);
  ...
  const onSavePress = useCallback(() => {
    ...
    Alert.alert('内容の確認', buildMealSummaryText(draft, recipient.name), [
      { text: '戻る', style: 'cancel' },
      { text: '保存する', onPress: () => void submitUpdate() },
    ]);
  }, [draft, isSignedIn, recipient, submitUpdate]);

  const setDraftSafe = useCallback(
    (action: SetStateAction<MealRecordDraft>) => {
      setDraft((prev) => {
        ...
        { text: 'キャンセル', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              setIsDeleting(true);
              try {
                await mealRecordsApi.destroy(recipientId, recordId);
                router.back();
              } catch (e) {
  ...
  }, [isDeleting, isSignedIn, mealRecordsApi, recordId, recipientId, router]);
